...
var app = new Vue({
    ...
    methods: {
        ...
        open:function () {
            ...
            $.post(global.url+"mission_new/handler?id=23",dataParam,function(res){
                ...
                if(res.code == 200){
                    ...
                    if(checkPlugin(version,version_check)){
                        personal.goldOpenTheTreasureBox('{"count":'+res.data.gold_tribute+'}');
                    }else{
                        $("#box-popup").show();
                    }
        ...
        newgoApp:function(index){
            ...
            if(url=="openVideo"){
                window.WebViewJavascriptBridge.callHandler('openIntroduceVideo','', function(responseData) {});
            }else{
                var _this = this;
                ...
                window.WebViewJavascriptBridge.callHandler('wechatLogin',"", function(responseData) {
                    var  responseData =  JSON.parse(responseData);
                    //数据请求
                    if(responseData.code==200){
        ...
        adClick:function () {
            personal.openGoogleAd("{adPageType:2}");
        }
    }
